...
# --- Configuration ---
CONFIG = {
    "START_URL": "https://dragonball.fandom.com/wiki/Main_Page",  # Starting point for Dragon Ball Fandom
    "BASE_URL": "https://dragonball.fandom.com",  # Base URL for Dragon Ball Fandom
    "ALLOWED_DOMAIN": "dragonball.fandom.com",  # Only crawl within this domain
    "MAX_IMAGES_TO_GATHER": 1050,  # Target number of images
    "OUTPUT_CSV_FILE": "dragonball_images.csv",  # Output CSV file
    ...
    "EXCLUDED_URL_PATTERNS": [
        ...
        "#", ".png", ".jpg", ".jpeg", ".gif", ".svg"  # Exclude direct links to images/files
    ]
}
...
def extract_image_and_text(soup: BeautifulSoup, page_url: str, base_url: str) -> List[Dict[str, str]]:
    ...
    figures = soup.find_all('figure', class_=lambda x: x and 'image' in x)
    for figure in figures:
        ...
        if img_tag and img_tag.get('src'):
            ...
            extracted_data.append({
                'page_url': page_url,
                'image_url': img_url,
                'text_surrogate': ' '.join(surrogate.split())
            })
...
def run_fandom_crawler(config: dict):
    ...
    print("--- Starting Dragon Ball Fandom Crawler ---")
